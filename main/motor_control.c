...
#define MAX_DUTY_CYCLE 255 // ≈ (255 * 0.10)
#define MIN_DUTY_CYCLE 192 // ≈ (255 * 0.75) 
...
// Turn the Motor ON
void motor_start() {
    gpio_set_level(MOTOR_IN1_PIN, 0); // Enable relay
    motor_running = true;
    ESP_LOGI("Motor Control", "Motor started.");
}
#endif 
...
// Turn the Motor OFF
void motor_stop() {
    gpio_set_level(MOTOR_IN1_PIN, 1); // Disable relay
    motor_running = false;
    ESP_LOGI("Motor Control", "Motor stopped.");
}
#endif 
...
void motor_control_task(void *pvParameters) {
    while (1) {
        if (current_state == WATER_FILL_MODE) {
            ...
            // Adjust motor speed dynamically (distance-based logic)
            ...
            if (current_level > (base_level + 5)) {
                ...
                // Calculate duty cycle with linear mapping
                ...
                ESP_LOGI("Motor Control", "Level Diff: %.2f, Duty Cycle: %d", level_diff, duty_cycle);
...
void motor_control_task_old(void *pvParameters) {
    while (1) {
        if (current_state == WATER_FILL_MODE) {
            if (!motor_running) {
                motor_start_forward(MIN_DUTY_CYCLE); // Start motor at minimum speed
...
void motor_control_task(void *pvParameters) {
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(MOTOR_CONTROL_TASK_DELAY_MS));
    }
}
#endif
